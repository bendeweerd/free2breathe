...
class NO2Sensor {
 private:
  ...
  const float Sf =
      -15;  // sensitivity in nA/ppm, ~1/2 stated sensitivity of -30 nA/ppm
 ...
 public:
  ...
  float read() {
    ...
    if (i >= numPolls) {
      ...
      amountNO2 = sensorValAvg / 1024;
      current = Vref / resValue;
      sensorSensitivity = Sf / 1000000000;

      PPM = amountNO2 * current / sensorSensitivity;  // TODO

      // print results
      Serial.print("PPM = ");
      Serial.print(PPM);
      Serial.print("\tnA = ");
      Serial.print(current * amountNO2 * 1000000000);  // TODO
      Serial.print("\tCounts = ");
